...
class Pullbacks_Trailing(Strategy):
    ...
    def init(self):
        ...
        self.sma20 = self.I(
            lambda x: ta.sma(pd.Series(x), length=20).to_numpy(),
            self.data.Close
        )
    ...
    def next(self):
        ...
        if pd.isna(self.sma200[-1]) or pd.isna(self.sma20[-1]):
            return
        
        cond_20_above_200 =  self.sma20[-1] > self.sma200[-1]
        cond_close_above_20 =  self.data.Close[-1] > self.sma20[-1] 
        cond_greenbar = self.data.Close[-1] > self.data.Open[-1] 
        cond_is_pullback = min(self.data.Low[-1], self.data.Low[-2]) < self.sma20[-1] 
        
        is_long = cond_20_above_200 and cond_close_above_20 and cond_greenbar and cond_is_pullback
        
        
        
        if pd.to_datetime(self.data.index[-1]) == pd.to_datetime('2003-04-02'):
            print('******* found ***********')
            print(f' cond_20_above_200: {cond_20_above_200}, cond_close_above_20: {cond_close_above_20}, cond_greenbar: {cond_greenbar}, cond_is_pullback: {cond_is_pullback}, in_position: {self.in_position}')
        ...
        if self.in_position:
            ...
            if self.data.Close[-1] < self.stop:
                ...
                self.stop = self.data.Low[-1]
        
        elif not self.in_position and is_long:
            
            if pd.to_datetime(self.data.index[-1]) == pd.to_datetime('2003-04-02'):
                print(f'======= equity ==== {self.equity} at {self.data.index[-1]}')
            ...
            self.stop = self.data.Low[-1]
            amount = self.equity * self.risk_prc
            size = amount // (self.data.Close[-1] - self.stop)
            self.buy(size=size, stop=self.stop)
            
            
    

bt = Backtest(spy, Pullbacks_Trailing, cash=25_000)
...
stats['_trades'].to_csv("trades_pullback_qqq.csv", index=False)
#tra = pd.read_csv("trades_spy.csv")
x = trades[["Size","EntryBar","ExitBar","EntryPrice","ExitPrice","SL","TP","PnL","Commission","ReturnPct","EntryTime","ExitTime"]]
print(x)